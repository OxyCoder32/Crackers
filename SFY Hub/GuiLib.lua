...
-- Color Theme (Ekuvé Inspired - Sleek Dark Theme)
local Theme = {
...
function GuiLibrary:CreateColorPicker(params, label, defaultColor, callback, isPremium, configKey)
    ...
    -- Create title label
    local titleLabel = Instance.new("TextLabel")
    titleLabel.Text = label .. (savedConfig and " ✓" or "")
    ...
    function colorPickerObj:SetLabel(newLabel)
        titleLabel.Text = newLabel .. (savedConfig and " ✓" or "")
    end
...
function GuiLibrary:CreateBadgedUpdateHistory(tab, historyData)
    ...
    local typeStyles = {
        bug = { emoji = "🐞", color = Color3.fromRGB(220, 80, 80) },
        fix = { emoji = "⚙️", color = Color3.fromRGB(220, 80, 80) },
        update = { emoji = "🆕", color = Color3.fromRGB(0, 150, 255) },
        improvement = { emoji = "⚡", color = Color3.fromRGB(255, 165, 0) },
        feature = { emoji = "✨", color = Color3.fromRGB(0, 200, 100) },
        security = { emoji = "🔒", color = Color3.fromRGB(150, 100, 255) },
        default = { emoji = "📝", color = Theme.TextSecondary }
    }
...
function GuiLibrary:CreateWelcomePanelTab(tab, scriptInfo)
    ...
    developerLabel.Text = "👑 DEVELOPED BY: " .. config.Developer
    ...
    welcomeTitle.Text = "🎮 WELCOME TO " .. string.upper(config.ScriptName:gsub(" SCRIPT", "")) .. " 🎮"
    ...
    featuresTitle.Text = "✨ FEATURE HIGHLIGHTS ✨"
    ...
    supportTitle.Text = "🛠️ GET SUPPORT & HELP 🛠️"
    ...
    helpButton.MouseButton1Click:Connect(function()
        ...
        helpTitle.Text = "🛠️ SUPPORT OPTIONS"
    ...
    function panelObj:UpdateConfig(newConfig)
        ...
        if developerLabel then developerLabel.Text = "👑 DEVELOPED BY: " .. config.Developer end
...
function GuiLibrary:CreatePlayerMarkerSystem(tab, groupName, defaultSettings, isPremium)
    ...
    local function createPlayerListItem(player)
        ...
        markButton.MouseButton1Click:Connect(function()
            ...
            if marked then
                self:ShowNotification("✅ Marked player: " .. player.Name, 3)
            else
                self:ShowNotification("❌ Unmarked player: " .. player.Name, 3)
            end
        end)
    ...
    clearButton.MouseButton1Click:Connect(function()
        ...
        self:ShowNotification("🧹 Cleared all player marks", 3)
    end)
    ...
    refreshButton.MouseButton1Click:Connect(function()
        ...
        self:ShowNotification("🔄 Player list refreshed", 3)
    end)
    ...
    print("• Green highlight for marked players")
    print("• Max mark distance: " .. MarkMaxDistance .. " studs")
    print("• Use the '" .. groupName .. "' section")
    print("========================================")
...
function GuiLibrary:SetElementValue(name, value)
    ...
    warn("❌ Element not found or doesn't support SetValue: " .. tostring(name))
    return false
end
...
function GuiLibrary:GetElementValue(name)
    ...
    warn("❌ Element not found or doesn't support GetValue: " .. tostring(name))
    return nil
end

-- List all registered elements (for debugging)
function GuiLibrary:ListElements()
    print("📋 Registered GUI Elements:")
...
function GuiLibrary:Set(name, value)
    ...
    warn("❌ Element not found: " .. tostring(name))
    return false
end
...
_G.Set = function(name, value)
    ...
    warn("❌ GUI Element not found: " .. tostring(name))
    return false
end
